 ...
 * Trait HasValidation
 *
 * Provides comprehensive validation functionality for models,
...
trait HasValidation
{
     ...
     *
     * @return array An associative array of attribute names and their error messages.
     */
    ...
    public function validate(string $context = 'create'): bool
    {
        foreach ($this->validates() as $attribute => $rules) {
            ...
            foreach ($rules as $rule) {
                ...
                if ($ruleName === self::VALIDATE_MIN && strlen((string)$value) < $rule['min']) {
                    $this->addErrorByRule($attribute, self::VALIDATE_MIN, $customMessage, ['min' => $rule['min']]);
                }

                if ($ruleName === self::VALIDATE_MAX && strlen((string)$value) > $rule['max']) {
